...
**Q.** `multi-user.target` 의 multi-user 란 무엇인가.

**A.** **여러 사용자가 동시에 한 시스템을 각자의 자격으로 사용할 수 있는 성질**이다. 유닉스·리눅스의 근본 설계 원칙이며, `multi-user.target` 은 **"그렇게 쓸 수 있는 상태에 도달했다"** 는 뜻이다.

---

### 1. 왜 이런 개념이 생겼는가

1960~70년대에는 컴퓨터가 극도로 비쌌다. 한 대를 한 사람이 독점하는 것은 낭비였으므로, **여러 사람이 각자 단말기(터미널)로 접속해 한 대를 나눠 쓰는** 방식이 필요했다.
...
```
        ...
        └──┬───┬───┬──┘
     ┌─────┘   │   └─────┐
  단말기A   단말기B   단말기C     ← 각자 자기 작업
   (김씨)    (이씨)    (박씨)
```

- 이 방식을 **시분할**(**time-sharing**) 이라 한다 — CPU 시간을 잘게 쪼개 번갈아 배정해, 각 사용자에게 **자기가 독점하는 것처럼** 보이게 함
- 여기서 쓰던 단말기가 [[#F-1. 환경변수·셸·tty 와 `su -` 가 환경을 초기화하는 이유]] 의 **tty**(전신 타자기)다
- 유닉스가 이 환경에서 태어났고, 리눅스가 그 설계를 이어받았다

#### 대조 — 단일 사용자 시스템

- MS-DOS·초기 Windows(3.1, 95/98)는 **단일 사용자** 시스템이었다
- 사용자 계정·파일 소유권·권한 개념이 없거나 형식적이었다
- 그래서 아무 프로그램이나 시스템 파일을 마음대로 고칠 수 있었다 → 보안 취약의 근원

---

### 2. 다중 사용자를 구현하려면 무엇이 필요한가

"여러 사람이 동시에" 를 성립시키려면 다음이 모두 갖춰져야 한다. **리눅스 학습 범위의 상당 부분이 사실 이 목록이다.**

| 필요 조건 | 리눅스의 구현 | 실습 위치 |
| --- | --- | --- |
| **사용자 구별** | UID·GID, `/etc/passwd`·`/etc/group` | [[LAB/03-user-group-permission]] 3절 |
| **인증** | 비밀번호 해시(`/etc/shadow`), PAM | [[LAB/03-user-group-permission]] 4~5절 |
| **파일 소유권과 권한** | 소유자·그룹·기타 × rwx, 특수권한, ACL | [[LAB/03-user-group-permission]] 8~10절 |
| **개인 공간** | 각자의 홈 디렉터리 `/home/<사용자>` | [[LAB/03-user-group-permission]] 3절 |
| **개인 환경** | 사용자별 환경변수·초기화 파일 | [[#F-2. 로그인 셸과 비로그인 셸의 차이]] |
| **동시 접속 경로** | 여러 tty·pts, SSH | [[#K-1. 소켓이란 무엇인가 — 파일 디스크립터와 함께]] |
| **프로세스 격리** | 프로세스마다 별도 메모리·FD 테이블 | [[#K-1. 소켓이란 무엇인가 — 파일 디스크립터와 함께]] |
| **자원 배분** | 스케줄러, `nice`, `ulimit`, 디스크 쿼터 | [[LAB/06-process-scheduling-diagnosis]] 4절, [[LAB/05-disk-lvm-raid-swap-quota]] 9절 |
| **권한 상승 통제** | `su`·`sudo`·wheel 그룹 | [[#C-1. wheel 그룹이란 무엇이며 sudo 와 어떤 관계인가]] |
| **행위 추적** | `/var/log/secure`, `wtmp`, auditd | [[LAB/10-security-firewall-selinux]] 5절 |

즉 **"다중 사용자"는 한 가지 기능이 아니라, 이 모든 장치가 함께 만들어 내는 성질**이다.
...
last | head
id
ps -eo user,tty,cmd --sort=user | head -20
```

- `who` : 현재 로그인한 사용자와 각자의 **tty**·접속 시각·출처 IP
- `w` : `who` + **각 사용자가 무엇을 실행 중인지** + 부하 평균
- `users` : 로그인한 사용자 이름만 한 줄로
- `last` : 과거 로그인 이력 (`/var/log/wtmp` 기록)
- `ps -eo user,tty` : 프로세스가 **누구 소유이고 어느 터미널에 붙었는지**

지금 실습 환경에서는 직렬 콘솔과 SSH 로 **동시에 두 세션**이 열려 있으므로, `who` 에 두 줄이 나온다. [[#F-4. `pstree` 출력 — 최소 설치 Rocky 9 의 프로세스 전수 해설]] 에서 `login→bash` 와 `sshd-session→bash` 가 함께 보였던 것이 바로 그 상태다.

```text
admin1   tty1         2026-09-04 13:00
admin1   pts/0        2026-09-04 13:20 (192.168.64.1)
```

- `tty1` = 콘솔 접속, `pts/0` = 원격 접속

---

### 4. `multi-user.target` 의 의미

이 맥락에서 타겟 이름이 이해된다 ([[#J-2. 타겟(target)과 런레벨(runlevel)이란 정확히 무엇인가]]).

| 타겟 | 사용자 관점의 상태 |
| --- | --- |
| `rescue.target` (런레벨 1) | **단일 사용자 모드** — root 한 명만, 네트워크 없음, 서비스 최소 |
| `multi-user.target` (런레벨 3) | **다중 사용자 모드** — 네트워크·로그인 서비스 가동, 여러 사용자 접속 가능 |
| `graphical.target` (런레벨 5) | 다중 사용자 + GUI 로그인 |

- **런레벨 1 을 "단일 사용자 모드"(single-user mode)라 부르는 이유** : 복구를 위해 일부러 다른 사용자를 막고 root 만 들어오게 하기 때문. 네트워크도 끊어 외부 접속을 차단한다
- 반대로 `multi-user.target` 은 **네트워크와 `sshd`·`getty` 가 떠서 누구나 인증을 거쳐 들어올 수 있는 상태**

---

### 5. 혼동하기 쉬운 이웃 개념 (시험 출제)

| 용어 | 뜻 | 예 |
| --- | --- | --- |
| **다중 사용자** (multi-user) | 여러 **사용자**가 동시에 사용 | 김씨·이씨가 각자 SSH 접속 |
| **다중 작업** (multi-tasking) | 여러 **작업(프로세스)** 을 동시에 처리 | 한 사용자가 편집기와 컴파일을 동시에 |
| **다중 처리** (multi-processing) | 여러 **CPU·코어**를 함께 사용 | `nproc` = 2 인 이 VM |
| **다중 스레드** (multi-threading) | 한 프로세스 안에서 여러 실행 흐름 | `pstree` 의 `{NetworkManager}` |
| **다중 플랫폼** (multi-platform) | 여러 **하드웨어 아키텍처**에서 동작 | x86_64·aarch64 |

- 다중 사용자와 다중 작업은 **별개 개념**이다. 단일 사용자라도 다중 작업은 가능하다
- 다만 다중 사용자를 구현하려면 다중 작업이 **전제로 필요**하다
...
| --- | --- |
| 다중 사용자·다중 작업 | 위 설명 |
| 다중 처리·다중 스레드 | SMP(**S**ymmetric **M**ulti-**P**rocessing) 지원 |
| 다중 플랫폼 | 이식성(portability)이 높음 — C 언어로 작성 |
| 계층적 파일시스템 | `/` 를 뿌리로 하는 단일 트리 ([[#B-2. 마운트란 무엇인가]]) |
| 뛰어난 네트워크 지원 | TCP/IP 를 커널에 내장 |
| 가상 메모리 | 물리 메모리보다 큰 주소 공간, 스왑 활용 |
| 오픈 소스 | GPL 라이선스, 소스 공개 |
| 셸·스크립트 | 명령 조합으로 자동화 |
| 뛰어난 보안성 | 권한 분리, SELinux, 방화벽 |

#### 운영체제 처리 방식 분류

| 방식 | 뜻 |
| --- | --- |
| **일괄 처리** (batch) | 작업을 모아 한꺼번에 처리. 대화형 아님 |
| **시분할** (time-sharing) | CPU 시간을 쪼개 여러 사용자에게 배분 → **리눅스** |
| **실시간** (real-time) | 정해진 시간 안에 반드시 응답 |
| **분산 처리** (distributed) | 여러 컴퓨터가 협력 |

---

### 6. 다중 사용자 환경의 실무 과제

여러 사람이 함께 쓰면 생기는 문제와 그 대응이 곧 시스템 관리 업무다.

| 문제 | 대응 | 실습 |
| --- | --- | --- |
| 남의 파일을 읽거나 지움 | 권한·특수권한(SetGID·Sticky)·ACL | [[LAB/03-user-group-permission]] 9~10절 |
| 한 사용자가 CPU 를 독점 | `nice`·`renice`, cgroup | [[LAB/06-process-scheduling-diagnosis]] 4절 |
| 한 사용자가 디스크를 가득 채움 | 디스크 쿼터 | [[LAB/05-disk-lvm-raid-swap-quota]] 9절 |
| 프로세스를 무한 생성 | `ulimit -u`, `limits.conf` | [[LAB/03-user-group-permission]] 5-5 |
| 권한 없는 관리 작업 시도 | `sudo` 로 명령 단위 위임 | [[#C-1. wheel 그룹이란 무엇이며 sudo 와 어떤 관계인가]] |
| 누가 무엇을 했는지 불명확 | 로그·감사 기록 | [[LAB/10-security-firewall-selinux]] 5절 |

절차서의 시나리오가 **개발 2명·운영 1명·관리자 1명의 팀 서버**로 설정된 이유가 이것이다 — 다중 사용자 환경의 과제를 실제로 겪어 보기 위함.
